...
export function registerHandlers(
	...
	io.on("connection", (socket: BingoSocket) => {
		...
		socket.on("callLine", (payload: CallLinePayload) => {
			...
			// Validate that the player actually has a complete line
			const drawnSet = new Set(room.getDrawnNumbers());
			const hasValidLine = player.cards.some((card) =>
				hasLineServer(card, drawnSet),
			);
			if (!hasValidLine) {
				socket.emit("error", {
					code: ErrorCode.LINE_NOT_COMPLETE,
					message: "No tenés una línea completa",
				});
				return;
			}
		...
		socket.on("callBingo", (payload: CallBingoPayload) => {
			...
			const drawnSet = new Set(room.getDrawnNumbers());
			const hasValidBingo = player.cards.some((card) =>
				hasBingoServer(card, drawnSet),
			);
			if (!hasValidBingo) {
				socket.emit("error", {
					code: ErrorCode.BINGO_NOT_COMPLETE,
					message: "No tenés bingo completo",
				});
				return;
			}
...
function getErrorMessage(code: string): string {
	const messages: Record<string, string> = {
		...
		[ErrorCode.NOT_DISPENSADOR]:
			"Solo el dispensador puede realizar esta acción",
		...
		[ErrorCode.LINE_NOT_COMPLETE]: "No tenés una línea completa",
		[ErrorCode.BINGO_NOT_COMPLETE]: "No tenés bingo completo",
	};
