...
import { hasLine, hasBingo } from "@/types";
...
export default function GamePage({ params }: { params: { gameId: string } }) {
	...
	useEffect(() => {
		...
		const cleanupNumberUnmarked = onEvent(
			"numberUnmarked",
			({ number, drawnNumbers }) => {
				setGameState((prev) => (prev ? { ...prev, drawnNumbers } : null));
				sessionStorage.setItem(
		...
		const cleanupGameEnded = onEvent(
			"gameEnded",
	...
	useEffect(() => {
		if (role === "dispensador") {
			...
			const storedLineCalled =
				sessionStorage.getItem("bingo_lineCalled") === "true";
			const storedBingoCalled =
				sessionStorage.getItem("bingo_bingoCalled") === "true";
		...
		return (
			<main className="flex min-h-screen items-center justify-center px-4">
				<div className="text-center">
					...
					<a href="/" className="text-wood-dark hover:underline font-medium">
						Volver al inicio
					</a>
				</div>
			</main>
